...
**公开测试版：** `v0.1.2` 是当前补丁版本，`v0.1.0` 是首个公开版本。测试期间，接口和来源可用性仍可能调整。
...
Loyalty Radar 不会默认逐条前往官网核验。它负责整理公开来源中的说法、标注证据质量，并保留原始链接供用户判断。[公开报告归档](https://lonelydoctor.github.io/loyalty-radar/zh-CN/)只发布经过审核和清洗的报告；第一份报告通过审核前，页面会如实显示空状态。

## 30 秒快速开始

选择一种安装形态即可，所有方式共用同一个 Skill 和 Python 实现。

### Agent Skills 一条命令安装

兼容 Agent 的最短且已测试安装方式是：

```bash
npx skills add lonelydoctor/loyalty-radar -g
```

安装后，直接让 Agent 生成过去两周的常旅客情报报告。
...
### Agent Skills / Claude 兼容 Skill 手动安装
...
| 面向个人用户的定时与推送 | v0.1.x 不包含；个人报告由 Agent 或用户手动触发 |
| 托管服务与遥测 | 不包含 |
...
GitHub 上的每周健康工作流只探测有限样本，不会抓取完整目录。样本会在可由脚本探测的来源包之间均衡分配，并按 ISO 周轮换，避免行业源或较低优先级来源长期被 P0 前缀遮蔽。工作流只保存健康元数据，并在 P0 来源连续两次探测失败后创建去重 Issue。端点失败只表示当时的可访问性，不代表该来源没有发布新闻。

FlyerTalk 订阅源始终优先直连。直连失败且来源声明 `fallback_provider: feedly-public` 时，Loyalty Radar 可以读取 Feedly 的公开缓存流，同时保留原始帖子链接和发布时间。报告会单独统计这类回退成功；发送给 Feedly 的只有已经公开的 RSS URL。若不接受该第三方数据边界，可在自定义来源包中删除该字段。
...
- 来源元数据与公开 URL；
- 根据这些已提交来源元数据生成的视觉素材；
- 经过批准并清洗为 `loyalty-radar-public-report/v1` 的公开报告 JSON；
- 仅位于 `tests/` 的明确非生产测试夹具，运行时代码和公开页面不会读取它们。

私有审计 JSON、来源正文、个人画像、持卡信息和翻译缓存始终留在本地。GitHub Pages 只能发布经过审核的公开投影，其中仅包含目标语言事件文本、来源名称、HTTP(S) 链接、发布时间、标签、数字锚点和汇总采集健康。站点构建会拒绝 `original`、原始内容、个人画像和完整健康明细字段，也不会把输入 JSON 复制到 `docs/site`。
...
根页面、[`/en/`](https://lonelydoctor.github.io/loyalty-radar/en/) 与 [`/zh-CN/`](https://lonelydoctor.github.io/loyalty-radar/zh-CN/) 已改为报告优先的首页与归档。页面只渲染 `public-briefs/<ISO-WEEK>/report.json` 中使用 `loyalty-radar-public-report/v1` 契约且经过批准的文件；没有已批准报告时，会明确显示空状态，不会补造新闻。

双语来源目录迁移到 `/en/sources/` 与 `/zh-CN/sources/`，同时保留 `/sources/` 作为直接进入英文目录的地址。目录从仓库提交的 59 个真实来源配置离线重建，展示来源名称、公开 URL、来源包、优先级、语言、地区和声明的采集方式。进入目录不代表来源当前可访问，也不代表它今天发布了新闻。
...
使用以下命令重新生成事实型 1280 x 640 社交预览，并同步重建 Pages 中的副本：
...
预览图只使用稳定的产品事实：59 个来源、14 天证据窗口、C 端与生态两条主线，以及 Plugin / Agent Skill / Python CLI 三种形态；它不会读取任何报告事件。

公开可见页面只显示目标语言，绝不回退到原文。每份已发布报告都会完整展示生成时间、准确的过去 14 天证据窗口、未来 60 天观察期、采集健康、C 端与生态两条主线、完整事件标题、来源时间和链接，以及行动、风险和置信标签；来源正文不会进入页面。
